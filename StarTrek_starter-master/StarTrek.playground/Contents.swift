...
let femaleHuman = humanOfficers.filter{femaleOfficers.contains($0)}
print(femaleHuman.count)
print(Float(femaleHuman.count) / Float(humanOfficers.count))



// Q2: How many first officers are human?
let firstHuman = firstOfficers.keys.filter{ humanOfficers.contains($0)}
print(firstHuman)
...
print(allCaps.map{$0.name}.sorted())
...
let vulcanFirst: [String] = firstOfficers.keys.filter{vulcanOfficers.contains($0)}
print(vulcanFirst)
let vulcanFirstOfficers: [Officer] = vulcanFirst.compactMap{Officer(name: $0, ship: firstOfficers[$0]!)}
...
let vulcanFirstLessThan5 = vulcanFirst.reduce(0) {$1.count < 5 ? $0+1 : $0}
print(vulcanFirstLessThan5)
